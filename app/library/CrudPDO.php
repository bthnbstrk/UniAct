...
//            return ['status' => TRUE];
    ...
    public function userLogout()
    {
        ...
        setcookie("usersLogin", json_encode($users), strtotime("-30 day"), "/");
        Header("Location:/default/login");
        exit;
    }

    public function getEvent($id_activity)
    {
        $stmt = $this->db->prepare('Select id_user,comment from comments_activities Where id_activity=?');
        $stmt->execute(array($id_activity));
        return $stmt;
    }

    public function getLike($id_activity)
    {
        $stmt = $this->db->prepare('Select id_user from liked_activities Where id_activity=?');
        $stmt->execute(array($id_activity));
        return $stmt;
    }

    public function usersLogin($email, $password, $remember_me)
    {
        ...
        try {
            ...
            if (isset($_COOKIE['usersLogin'])) {
                $stmt->execute([$email, md5(openssl_decrypt($password, "AES-128-ECB", "admins_coz1"))]);
            } else {
            ...
            if ($stmt->rowCount() == 1) {
                ...
                $_SESSION["users"] = [
                    ...
                    "id_univercity" => $row['id_univercity'],
                ...
                if (!empty($remember_me) AND empty($_COOKIE['usersLogin'])) {
                    ...
                    $users =
                        [
                            "email" => $email,
                            "password" => openssl_encrypt($password, "AES-128-ECB", "admins_coz1")
                        ];
